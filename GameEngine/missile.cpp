...
Missile::Missile(double speed0,Point p,double angle0,HitPoint* hit_point0,Graphic *graphic0,int damage0,
...
void Missile::ChangeStatus(double time, Game &my_game)
{
    ...
    if (target!=NULL){
        if (turned_angle>=miss_target_angle||target->IsDestroyed())target=NULL;
